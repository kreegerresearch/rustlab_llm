...
Finite-difference estimate at $W_1(2, 1)$: ${fd:%.6f}$ vs analytic ${dL_dW1(2, 1):%.6f}$ — error ${err:%.2e}$ (well under $\varepsilon^2$).
...
Shapes: $\bar{\mathbf{W}} \in \mathbb{R}^{${size(dL_dW, 1)} \times ${size(dL_dW, 2)}}$, $\bar{\mathbf{b}} \in \mathbb{R}^{1 \times ${size(dL_db, 2)}}$, $\bar{\mathbf{x}} \in \mathbb{R}^{1 \times ${size(dL_dx, 2)}}$ — all match their forward-pass counterparts.
...
Cross-entropy loss $L = ${L_ce:%.4f}$. Analytical $\bar{z}_3 = ${dL_dz_analytic(3):%.4f}$, finite-difference $= ${fd:%.4f}$ — match to ${abs(fd - dL_dz_analytic(3)):%.2e}$.
...
X = randn(T2, d_model2) * 0.5;
W_Q = randn(d_model2, d_k2) * 0.5;
W_K = randn(d_model2, d_k2) * 0.5;
W_V = randn(d_model2, d_v2) * 0.5;
