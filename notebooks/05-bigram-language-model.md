...
the **conditional entropy** of the next token given the current one. For our `"abcbabcba"` example you measured ${mean_ce:%.4f}$ nats $\approx ${mean_ce/log(2):%.4f}$ bits per token. Shannon's source coding theorem says no compressor that sees only the current token can do better — this is the **fundamental bit floor for any Markov-1 model** of this corpus.

**Perplexity = $2^H$ (or $e^H$).** The perplexity ${ppl:%.3f}$ you printed above is the **effective branching factor**: the model is, on average, as uncertain as if it had to choose uniformly among ${ppl:%.3f}$ tokens. A perfect bigram on this corpus would reach $2^{H(X_{t+1}\mid X_t)} = 2^{0.5} \approx 1.414$ — and it does. There is no slack left to close at the Markov-1 horizon.
