...
Built-in `layernorm` matches the by-hand computation up to the regularisation $\epsilon$ — they agree to ${max(abs(ln_v_manual - ln_v_call)):%.2e}$. Output mean is exactly 0; (population) std is exactly 1.
