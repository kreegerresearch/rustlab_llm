...
└── 01-tokens-and-encoding/
    ├── char_frequencies.rlab
    ├── one_hot_encoding.rlab
    └── *.svg              # output from `rustlab run *.rlab` (gitignored)
```

Subdirectories track the notebook slugs one-for-one. As `.rlab` scripts
...
by the renderer into the published site. The `.rlab` scripts are *parallel*
